            ...
            if results.multi_face_landmarks:
                for facelm in results.multi_face_landmarks:
                    for id, lm in enumerate(facelm.landmark):
                        if id in chosen_left_eye_idxs :
                            ih, iw, ic = img.shape
                            x, y = int(lm.x * iw), int(lm.y * ih)
                            cv2.circle(img , (x,y),1 , (0,255,0),1)
                        if id in chosen_right_eye_idxs :
                            ih, iw, ic = img.shape
                            x, y = int(lm.x * iw), int(lm.y * ih)
                            cv2.circle(img, (x, y), 1, (0, 255, 0), 1)
                ...
                if EAR < 0.15:
                    if start is None:
                        ...
                        if start is not None:
                            ...
                            if cal > 1:
                                start = time.time() + 1.3
                                filename = "D:\\driver_droness\\wakeup.mp3"
                ...
                cv2.putText(img,"Your eyes closed in: {tong}",(10,130),cv2.FONT_HERSHEY_SIMPLEX,0.7,(0,255,0),2)
                cv2.imshow("hii",img)
                key = cv2.waitKey(1)
